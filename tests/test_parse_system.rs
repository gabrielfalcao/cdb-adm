...
fn load_print_system() -> String {
    let path = folder_path!().join("launchctl-print-system");
    path.read().expect(&format!("contents of {}", &path))
}
fn load_print_system_disabled() -> String {
    let path = folder_path!().join("launchctl-print-system-disabled");
    path.read().expect(&format!("contents of {}", &path))
}
#[test]
fn test_parse_services_not_disabled() -> Result<()> {
    let services = parse_services(&load_print_system(), false)?
        .iter()
        .map(|(_, _, service, _)| service.to_string())
        .collect::<Vec<String>>();
...
#[test]
fn test_parse_services_disabled() -> Result<()> {
    let services = parse_services(&load_print_system_disabled(), true)?
        .iter()
        .map(|(_, _, service, _)| service.to_string())
        .collect::<Vec<String>>();
